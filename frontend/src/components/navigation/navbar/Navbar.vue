...
<template>
  <header v-if="!auth.isAuthenticated"
    ...
    <div class="shrink-0 inline-flex justify-center items-center ">
      <SidebarToggle class=""/>
    ...
    <nav
      ...
      <NavLink v-for="navLink in navLinks" :key="navLink.id" :href="navLink.href"
               class="inline-flex justify-center items-center w-fit px-2 py-1.5
        ...
        <IconLabel>
          ...
          <template #label>
            <div v-text="navLink.label.value"
                 class="inline-block h-[2lh] content-center"/>
          </template>
        </IconLabel>
      </NavLink>
    </nav>
    <div class="inline-flex justify-between items-center gap-5">
      <LanguageSelect class="font-semibold" selected-language-classes="md:max-xl:hidden"/>
