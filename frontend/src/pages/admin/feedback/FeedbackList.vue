...
import { BackIcon, DeckFeedbackIcon, TrashIcon, RefreshIcon, ArrowIcon, StarIcon, Searchbar, Button, IconButton } from '@/shared/ui';
import { getAllFeedback, deleteFeedback, type AdminFeedback } from '@/entities/admin/api';
...
const { data, isLoading, error, refetch } = useQuery({
  queryKey: computed(() => ['admin-feedback', searchQuery.value, currentPage.value, limit.value]),
        ...
        <div class="rounded-3xl border border-default bg-primary shadow-xl p-10">
          <div class="flex items-start gap-5 mb-6">
            ...
            <div class="p-4 rounded-2xl border border-default">
              <DeckFeedbackIcon class="icon-static w-14"/>
            </div>
            <div class="flex-1">
              <h1 class="text-4xl font-bold tracking-tight">Управление отзывами</h1>
              <p class="mt-2 text-lg text-muted leading-relaxed">
                Модерация отзывов пользователей на публичные колоды
              </p>
            </div>
            <button
              @click="refetch()"
          ...
          <!-- Search -->
          <div class="mb-6">
            <Searchbar v-model="searchQuery" placeholder="Поиск по комментарию, пользователю или колоде..."/>
          </div>

          <!-- Loading State -->
          <div v-if="isLoading" class="p-8 rounded-2xl border border-default bg-tertiary text-center shadow-lg">
            <p class="text-muted text-xl">Загрузка отзывов...</p>
          </div>

          <!-- Error State -->
          <div v-else-if="error" class="p-8 rounded-2xl border border-default bg-tertiary text-center shadow-lg">
            <p class="text-muted text-xl mb-4">Ошибка при загрузке отзывов</p>
            <p class="text-base text-muted">{{ error }}</p>
          </div>

          <!-- Feedback Table -->
          <div v-else-if="feedbacks.length > 0" class="rounded-2xl border border-default bg-tertiary shadow-lg overflow-hidden">
            <div class="overflow-x-auto">
              <!-- Grid-based layout -->
              <div class="min-w-[1200px]">
                <!-- Header -->
                <div class="grid grid-cols-[minmax(60px,auto)_minmax(100px,auto)_minmax(250px,2fr)_minmax(180px,1fr)_minmax(180px,1fr)_minmax(180px,1fr)_minmax(80px,auto)] gap-4 bg-primary border-b border-default px-6 py-5">
                  <div class="text-base font-semibold">ID</div>
                  <div class="text-base font-semibold">Рейтинг</div>
                ...
                <div
                  ...
                  :key="feedback.id"
                  class="grid grid-cols-[minmax(60px,auto)_minmax(100px,auto)_minmax(250px,2fr)_minmax(180px,1fr)_minmax(180px,1fr)_minmax(180px,1fr)_minmax(80px,auto)] gap-4 border-b border-default hover:bg-primary/50 transition-colors px-6 py-5 items-center"
                >
                  <div class="text-base font-medium">{{ feedback.id }}</div>
                  
                  <div class="flex items-center gap-1">
                    <StarIcon class="icon-static w-5 h-5"/>
                    <span class="text-base font-semibold">{{ feedback.rating }}</span>
                  </div>
                  
                  <div class="text-base truncate" :title="feedback.content || 'Без комментария'">
                    {{ truncateText(feedback.content, 60) || 'Без комментария' }}
                  </div>
                  
                  <div class="min-w-0">
                    <div class="font-medium text-base truncate" :title="feedback.user.username">
                      {{ feedback.user.username }}
                    </div>
                    <div class="text-sm text-muted truncate" :title="feedback.user.email">
                      {{ feedback.user.email }}
                    </div>
                  </div>
                  
                  <div class="min-w-0">
                    <div class="font-medium text-base truncate" :title="feedback.deck.name">
                      {{ feedback.deck.name }}
                    </div>
                    <div class="text-sm text-muted truncate" :title="feedback.deck.user.username">
                      от {{ feedback.deck.user.username }}
                    </div>
                  </div>
                  
                  <div class="text-base text-muted">{{ formatDate(feedback.createdAt) }}</div>
          ...
          <div v-else class="p-8 rounded-2xl border border-default bg-tertiary text-center shadow-lg">
            <p class="text-muted text-xl">Отзывы не найдены</p>
          </div>
        </div>
      </section>
    </div>
